...
# =====================================
# 6) Obtener eventos para el select del buzón
# =====================================

@perfil_bp.route("/eventos_buzon", methods=["GET"])
@jwt_required()
def eventos_buzon():
    """
    Devuelve los eventos relacionados con el asistente logueado
    para mostrarlos en el select de 'Evento relacionado' del buzón.
    """
    identidad = get_jwt_identity()
    id_persona = identidad.get("id_persona")
    ...
    # Tomamos TODOS los eventos donde el asistente tenga un registro (pasados y futuros)
    registros = (
        Registro.query
        .join(Evento, Registro.id_evento == Evento.id_evento)
        .filter(Registro.id_asistente == asistente.id_asistente)
    ...
    # Por si acaso hubiera duplicados del mismo evento
    # (normalmente no, pero lo hacemos "bonito")
    vistos = set()
    eventos_unicos = []
    ...
    return jsonify({"ok": True, "eventos": eventos_unicos}), 200
